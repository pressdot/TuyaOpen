...
#define SDP_TEMPLATE_PART_MEDIA_HEADER \
    "m=%s 9 %s%s\r\n"
...
#define SDP_TEMPLATE_PART_MEDIA_TRANSPORT_CANDIDATE\
    "%s"
...
#define SDP_TEMPLATE_PART_MEDIA_MID \
    "a=mid:%s\r\n"

#define SDP_TEMPLATE_PART_MEDIA_DIRECTION\
    "a=%s\r\n"

#define SDP_TEMPLATE_PART_MEDIA_MSID \
    "a=msid:%s %s\r\n"

#define SDP_TEMPLATE_PART_MEDIA_RTCP_MUX\
    "a=rtcp-mux\r\n"

#define SDP_TEMPLATE_PART_MEDIA_SSRC\
    "a=ssrc:%u cname:%s\r\n"
...
#define SDP_TEMPLATE_PART_MEDIA_APPLICATION_RTPMAP \
    "a=rtpmap:%d %s %d\r\n"

#define SDP_TEMPLATE_PART_MEDIA_AUDIO_RTPMAP \
    "a=rtpmap:%d %s/%d\r\n"

#define SDP_TEMPLATE_PART_MEDIA_VIDEO_RTPMAP \
    "a=rtpmap:%d %s/%d\r\n"

#define SDP_TEMPLATE_PART_MEDIA_VIDEO_RTPFB_FIR\
    "a=rtcp-fb:%d ccm fir\r\n"

#define SDP_TEMPLATE_PART_MEDIA_VIDEO_RTPFB_NACK\
    "a=rtcp-fb:%d nack\r\n"

#define SDP_TEMPLATE_PART_MEDIA_VIDEO_RTPFB_PLI\
...
#define SDP_TEMPLATE_PART_MEDIA_APPLICATION_KEY \
    "a=aes-key:%s\r\n"

rtc_audio_codec_t default_audio_rtpmaps[] = {
    {
        {NULL,NULL},
        "PCMU",
        0,
...
static int tuya_p2p_rtc_sdp_encode_media_audio(rtc_sdp_t *sdp, char *type, char *mid, char *buf, int size){
    ...
    // media direction
    ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_DIRECTION, tuya_p2p_rtc_media_direction_str(sdp->audio.direction));
    if(ret < 0 || ret >= remain){
        return -1;
    }
    already += ret;
    ...
    if(strcmp(type, "offer") == 0){
        ...
        QUEUE_FOREACH(q, &sdp->audio.codec_list){
            rtc_audio_codec_t *codec = QUEUE_DATA(q, rtc_audio_codec_t, queue);
            ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_AUDIO_RTPMAP, codec->pt, codec->name, codec->sample_rate);
            if(ret < 0 || ret >= remain){
                return -1;
            }
            already += ret;
        ...
        ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_AUDIO_RTPMAP, sdp->audio.negotiated_codec.pt, sdp->audio.negotiated_codec.name, sdp->audio.negotiated_codec.sample_rate);
        if(ret < 0 || ret >= remain){
            return -1;
        }
        already += ret;
...
static int tuya_p2p_rtc_sdp_encode_media_video(rtc_sdp_t *sdp, char *type, char *mid, char *buf, int size){
    ...
    // media direction
    ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_DIRECTION, tuya_p2p_rtc_media_direction_str(sdp->video.direction));
    if(ret < 0 || ret >= remain){
        return -1;
    }
    already += ret;
    ...
    if(strcmp(type, "offer") == 0){
        ...
        QUEUE_FOREACH(q, &sdp->video.codec_list){
            ...
            if(strcmp(codec->name, "rtx")){
                // rtpmap
                ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_VIDEO_RTPMAP, codec->pt, codec->name, codec->clock_rate);
                if(ret < 0 || ret >= remain){
                    return -1;
                }
                already += ret;
                ...
                // attr
                ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_VIDEO_ATTR, codec->pt, codec->profile_level_id);
                if(ret < 0 || ret >= remain){
                    return -1;
                }
                already += ret;
                ...
                ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_RTX, codec->pt, codec->clock_rate, codec->pt, codec->original_pt);
                if(ret < 0 || ret >= remain){
                    return -1;
                }
                already += ret;
        ...
        // rtpmap
        ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_VIDEO_RTPMAP, sdp->video.negotiated_codec.pt, sdp->video.negotiated_codec.name, sdp->video.negotiated_codec.clock_rate);
        if(ret < 0 || ret >= remain){
            return -1;
        }
        already += ret;
        ...
        // attr
        ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_VIDEO_ATTR, sdp->video.negotiated_codec.pt, sdp->video.negotiated_codec.profile_level_id);
        if(ret < 0 || ret >= remain){
            return -1;
        }
        already += ret;
        ...
        if(sdp->video.rtx_mode == RTX_MODE_SSRC_MULTIPLEX){
            // rtx
            ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_RTX, sdp->video.rtx_codec.pt, sdp->video.rtx_codec.clock_rate, sdp->video.rtx_codec.pt, sdp->video.negotiated_codec.pt);
            if(ret < 0 || ret >= remain){
                return -1;
            }
            already += ret;
    ...
    if(sdp->video.rtx_mode == RTX_MODE_SSRC_MULTIPLEX){
        // ssrc group
        ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_SSRC_GROUP, 
                sdp->video.negotiated_codec.ssrc, sdp->video.rtx_codec.ssrc,
                sdp->video.negotiated_codec.ssrc, sdp->cname,
...
static int tuya_p2p_rtc_sdp_encode_media_tuya(rtc_sdp_t *sdp, char *type, char *mid, char *buf, int size){
    ...
    if(strcmp(type, "offer") == 0){
        ...
        QUEUE_FOREACH(q, &sdp->tuya.codec_list){
            rtc_tuya_codec_t *codec = QUEUE_DATA(q, rtc_tuya_codec_t, queue);
            ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_APPLICATION_RTPMAP, codec->pt, codec->name, codec->channel_number);
            if(ret < 0 || ret >= remain){
                return -1;
            }
            already += ret;
        ...
        ret = snprintf(buf+already, remain, SDP_TEMPLATE_PART_MEDIA_APPLICATION_RTPMAP, sdp->tuya.negotiated_codec.pt, sdp->tuya.negotiated_codec.name, sdp->tuya.negotiated_codec.channel_number);
        if(ret < 0 || ret >= remain){
            return -1;
        }
        already += ret;
...
int tuya_p2p_rtc_sdp_init(rtc_sdp_t *sdp, char *session_id, char *local_id, char *fingerprint, char *ufrag, char *password, tuya_p2p_rtc_dtls_role_e dtls_role){
...
int tuya_p2p_rtc_sdp_add_audio_codec(rtc_sdp_t *sdp, char *name, int pt, uint32_t ssrc, int sample_rate, int channel_number){
    if (pt != 0) {
        return -1;
    }
...
int tuya_p2p_rtc_sdp_add_video_codec(rtc_sdp_t *sdp, char *name, int pt, uint32_t ssrc, int clock_rate, char *profile_level_id){
    ...
    QUEUE_FOREACH(q, &local_sdp->audio.codec_list){
        ...
        QUEUE_FOREACH(tmp_q, &remote_sdp->audio.codec_list){
            ...
            if((strncmp(codec->name, tmp_codec->name, sizeof(codec->name)) == 0) \
                ...
                snprintf(local_sdp->audio.negotiated_codec.name, \
                            sizeof(local_sdp->audio.negotiated_codec.name), \
                            "%s", codec->name);
                ...
                snprintf(remote_sdp->audio.negotiated_codec.name, \
                            sizeof(remote_sdp->audio.negotiated_codec.name), \
                            "%s", tmp_codec->name);
                goto finish_audio;
            }
        }
        ...
        QUEUE_FOREACH(tmp_q, &remote_sdp->video.codec_list){
            ...
            if((strncmp(codec->name, tmp_codec->name, sizeof(codec->name)) == 0) \
                ...
                snprintf(local_sdp->video.negotiated_codec.profile_level_id, \
                            sizeof(local_sdp->video.negotiated_codec.profile_level_id), \
                            "%s", codec->profile_level_id);
                snprintf(local_sdp->video.negotiated_codec.name, \
                            sizeof(local_sdp->video.negotiated_codec.name), \
                            "%s", codec->name);
                ...
                snprintf(remote_sdp->video.negotiated_codec.profile_level_id, \
                            sizeof(remote_sdp->video.negotiated_codec.profile_level_id), \
                            "%s", tmp_codec->profile_level_id);
                snprintf(remote_sdp->video.negotiated_codec.name, \
                            sizeof(remote_sdp->video.negotiated_codec.name), \
                            "%s", tmp_codec->name);
                goto finish_video;
            }
        }
