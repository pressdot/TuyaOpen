...
void ice_on_rx_data(pj_ice_strans *ice_st, unsigned comp_id, void *buffer, pj_size_t size, const pj_sockaddr_t *src_addr, unsigned src_addr_len);
...
int rtc_crypt_encrypt_aes_128_cbc(struct tuya_p2p_rtc_session *rtc, void *ctx, size_t length, unsigned char *iv, const unsigned char *input, unsigned char *output);
int rtc_crypt_decrypt_aes_128_cbc(struct tuya_p2p_rtc_session *rtc, void *ctx, size_t length, unsigned char *iv, const unsigned char *input, unsigned char *output);
...
static int tuya_p2p_process_signal_msg(char *msg, int msglen) 
{
    ...
    if (g_pRtcSession == NULL && strcmp(type, "offer") == 0) {
        ...
        if (!cJSON_IsArray(el_token) && !cJSON_IsObject(el_udp_token) && !cJSON_IsObject(el_tcp_token)) {
            ...
            if (cJSON_IsArray(el_token)) {
                ...
                if (token_str != NULL) {
                    snprintf(cfg.ice_server_tokens, sizeof(cfg.ice_server_tokens), "%s", token_str);//解析从云端发来的ICE服务器信息
        ...
        int peer_security_level = cJSON_IsNumber(el_security_level) ? el_security_level->valueint : 3/*TUYA_P2P_SECURITY_LEVEL_3*/;
    ...
    if (strcmp(type, "candidate") == 0) {
        ...
        int close_reason = cJSON_IsNumber(jclose_reason) ? jclose_reason->valueint : 99/*RTC_SESSION_CLOSE_REASON_UNDEFINED*/;
...
int ctx_session_add_remote_candidate(tuya_p2p_rtc_session_t *rtc, rtc_sdp_t *remote_sdp, char *candidate)
{
    ...
    if (parse_cand(NULL, &pjstrCand, &cand) != 0 /*|| cand.type == PJ_ICE_CAND_TYPE_HOST || cand.type == PJ_ICE_CAND_TYPE_RELAYED*/)
    {
        return -1;
    }
...
int ctx_session_send_sdp(tuya_p2p_rtc_session_t *rtc, rtc_session_cfg_t *cfg)
{
    ...
    if (jfrom == NULL || jto == NULL || jsession_id == NULL || jmoto_id == NULL || jtype == NULL ||
        jtrace_id == NULL || jis_pre == NULL || jp2p_skill == NULL || jheader == NULL) {
        goto finish;
    }
...
void ice_on_new_candidate(pj_ice_strans* ice_st, const pj_ice_sess_cand* cand, pj_bool_t last)
{
	...
	if (cand) {
		...
		PJ_LOG(4, (THIS_FILE, "%p: discovered a new candidate "
			"comp=%d, type=%s, addr=%s, baseaddr=%s, end=%d",
			pIceSession, cand->comp_id,
...
tuya_p2p_rtc_session_t *ctx_session_create(rtc_session_cfg_t *cfg, rtc_state_e state, int32_t *err_code) 
{
    ...
    int ret = 0;
    tuya_p2p_rtc_dtls_role_e local_dtls_role = DTLS_ROLE_CLIENT/*rtc->cfg.role == RTC_ROLE_CALLER ? DTLS_ROLE_BOTH : DTLS_ROLE_CLIENT*/;
    tuya_p2p_rtc_dtls_role_e remote_dtls_role = DTLS_ROLE_SERVER/*rtc->cfg.role == RTC_ROLE_CALLER ? DTLS_ROLE_BOTH : DTLS_ROLE_SERVER*/;
    ret = tuya_p2p_rtc_sdp_init(&rtc->local_sdp, cfg->session_id, cfg->local_id, "", cfg->ice_ufrag, cfg->ice_password, local_dtls_role);
    if (ret < 0) {
...
int ctx_session_channel_process_pkt(void *user, int length, const char *input, char *output)
{
    ...
    if ((msg_size > 0) && ((msg_size % keylen) == 0)) {
        ret = rtc_crypt_decrypt_aes_128_cbc(rtc, chan->aes_ctx_dec, msg_size, (unsigned char *)iv, (const unsigned char *)(encrypted + iv_size), (unsigned char *)decrypted);
...
int tuya_p2p_rtc_channels_init(tuya_p2p_rtc_session_t *rtc) 
{
    ...
    for (i = 0; i < rtc->cfg.channel_number + 1; i++) {
        ...
        ikcp_wndsize(chan->kcp, send_buf_size / 1600/*TUYA_MBUF_HUGE_SIZE*/, recv_buf_size / 1600/*TUYA_MBUF_HUGE_SIZE*/);
...
int32_t tuya_p2p_rtc_dosend_data(tuya_p2p_rtc_session_t *rtc, uint32_t channel_id, char *buf, int32_t len, int32_t timeout_ms) 
{
    if (rtc == NULL) {
    ...
    while (remain > 0) {
        ...
        int ret = rtc_crypt_encrypt_aes_128_cbc(rtc, chan->aes_ctx_enc, buflen, 
                                                (unsigned char *)tmp_iv,
                                                (const unsigned char *)decrypted,
                                                (unsigned char *)encrypted + iv_size);
    ...
    if (already > 0) {
        ...
        //tuya_p2p_log_error("rtc %08x channel %d send rc = %d\n", rtc->handle, channel_id, TUYA_P2P_ERROR_SESSION_CLOSED_TIMEOUT);
        return TUYA_P2P_ERROR_SESSION_CLOSED_TIMEOUT;
    } else {
...
int32_t tuya_p2p_rtc_dorecv_data(tuya_p2p_rtc_session_t *rtc, uint32_t channel_id, char *buf, int32_t *len, int32_t timeout_ms) 
{
    if (rtc == NULL) {
        return TUYA_P2P_ERROR_SESSION_CLOSED_TIMEOUT;
    }
...
int32_t tuya_p2p_rtc_dorecv_data2(tuya_p2p_rtc_session_t *rtc, uint32_t channel_id, char *buf, int32_t *len, int32_t timeout_ms) 
{
    int ret = 0;
    uint64_t begin_time = 0;
    int buflen = *len;
