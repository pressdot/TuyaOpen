...
*/
struct lgw_conf_board_s {
    bool            lorawan_public; /*!> 是否用于支持LoRaWAN公共网络，如果为 true，则表示板卡用于支持LoRaWAN公共网络；如果为 false，则表示板卡用于其他类型的网络*/
    uint8_t         clksrc;         /*!> 提供时钟给集中器的射频链的索引 */
    bool            full_duplex;    /*!> 表示SX1302板卡是否支持全双工模式，如果为 true，则表示板卡支持全双工模式；如果为 false，则表示板卡不支持全双工模式。 */
    lgw_com_type_t  com_type;       /*!> SPI or USB? */
    char            com_path[64];   /*!> 访问连接到SX1302的COM设备的路径 */
};
...
struct lgw_conf_rxrf_s {
    ...
    uint32_t                freq_hz;            /*!> 射频的中心频率 */
    float                   rssi_offset;        /*!> RSSI校正因子 */
    struct lgw_rssi_tcomp_s rssi_tcomp;         /*!> RSSI温度补偿系数 */
    lgw_radio_type_t        type;               /*!> 射频链的无线电类型，例如SX1255、SX1257等 */
    bool                    tx_enable;          /*!> 启用或禁用该射频链上的发射参数 */
    bool                    single_input_mode;  /*!> 针对SX1250的配置，用于将无线电配置为差分输入模式 */
};
...
struct lgw_conf_rxif_s {
    ...
    uint8_t     rf_chain;       /*!> 该IF链与哪个射频链（RF chain）相关联的整数 */
    int32_t     freq_hz;        /*!>表示IF链的中心频率 */
    uint8_t     bandwidth;      /*!> RX（接收）带宽，表示IF链的接收带宽。0 表示使用默认带宽。 */
    uint32_t    datarate;       /*!> RX数据速率，表示IF链的接收数据速率。0 表示使用默认数据速率 */
    uint8_t     sync_word_size; /*!> FSK同步字的大小，表示FSK调制的同步字节的数量。0 表示使用默认大小 */
    uint64_t    sync_word;      /*!> FSK同步字，以64位无符号整数的形式表示，采用右对齐方式。这个字段用于指定FSK调制的同步字 */
    bool        implicit_hdr;               /*!>表示LoRa服务中是否启用隐式头部 */
    uint8_t     implicit_payload_length;    /*!>表示隐式头部的负载长度，以字节为单位。0 表示使用默认长度 */
    bool        implicit_crc_en;            /*!> 是否启用隐式头部的CRC */
    uint8_t     implicit_coderate;          /*!> 隐式头部的编码率  */
};
