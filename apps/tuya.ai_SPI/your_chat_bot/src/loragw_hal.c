...
int lgw_rxrf_setconf(uint8_t rf_chain, struct lgw_conf_rxrf_s * conf) {
    ...
    CONTEXT_RF_CHAIN[rf_chain].rssi_tcomp.coeff_a = conf->rssi_tcomp.coeff_a;//RSSI 的温度补偿系数
    ...
    CONTEXT_RF_CHAIN[rf_chain].type = conf->type;//射频芯片的类型（SX1255、SX1257、SX1250）
...
int lgw_start(void) 
{
    ...
    /* Only SX1302 AGC firmware loading */
    fw_version_agc = 10; /* Default version for SX1302 */
    ...
    err = sx1302_agc_start(fw_version_agc, LGW_RADIO_TYPE_NONE, SX1302_AGC_RADIO_GAIN_AUTO, SX1302_AGC_RADIO_GAIN_AUTO, CONTEXT_BOARD.full_duplex, false);
    if (err != LGW_REG_SUCCESS) {
...
int lgw_stop(void) {
    ...
    /* Abort current TX if needed 中止当前tx*/
    for (i = 0; i < LGW_RF_CHAIN_NB; i++) {
...
int lgw_receive(uint8_t max_pkt, struct lgw_pkt_rx_s *pkt_data) {
    ...
    /* Record function start time 记录开始时间*/
    _meas_time_start(&tm);

    /* Get packets from SX1302, if any 从SX1302获取数据包*/
    ...
    current_temperature = 25.0; /* Default temperature for SX1302 only mode */

    /* Iterate on the RX buffer to get parsed packets遍历接收缓冲区 */
    for (nb_pkt_found = 0; nb_pkt_found < ((nb_pkt_fetched <= max_pkt) ? nb_pkt_fetched : max_pkt); nb_pkt_found++) {
        ...
        /* Appli RSSI offset calibrated for the board 应用RSSI偏移*/
    ...
    /* Remove duplicated packets generated by double demod when precision timestamp is enabled数据包去重 */
...
int lgw_send(struct lgw_pkt_tx_s * pkt_data) {
    ...
    if (pkt_data->modulation == MOD_LORA) {//若为lora模式，确认带宽、扩频因子、编码率、数据大小
    ...
    /* Send the TX request to the concentrator发送数据包 */
    err = sx1302_send(LGW_RADIO_TYPE_NONE, &CONTEXT_TX_GAIN_LUT[pkt_data->rf_chain], CONTEXT_LWAN_PUBLIC, &CONTEXT_FSK, pkt_data);
    if (err != LGW_REG_SUCCESS) {
