...
void* video_thread(void *arg)
{
    ...
    //     int ret = __read_one_frame_from_demo_video_file(video_buf + offset, file_size - offset, offset, &is_key_frame, &frame_len, &frame_start);
    //     if (ret) {
    //         break;
    //     }
    ...
    unsigned int *IskeyFrame, unsigned int *FramLen, unsigned int *Frame_start)
{
    ...
    {
        if (pVideoBuf[pos] == 0x00 && pVideoBuf[pos + 1] == 0x00 && pVideoBuf[pos + 2] == 0x00 && pVideoBuf[pos + 3] == 0x01) 
...
INT_T Demo_OnGetVideoFrameCallback(MEDIA_FRAME *pMediaFrame)
{
    ...
    int ret = __read_one_frame_from_demo_video_file(video_buf + offset, offset, file_size - offset, &is_key_frame, &frame_len, &frame_start);
    if (ret) {
        return -1;
    }
