...
#define P2P_EXT_HEAD_MAX_LEN    (sizeof(C2C_AV_TRANS_FIXED_HEADER) + EXT_PROTOCOL_V0_LEN)  //拓展视频头协议 V0 head+ext(8+4)+rtp_len
...
OPERATE_RET p2p_get_userinfo(INT_T session, INT_T p2pType)
{
    ...
    while (retry > 0){
        ...
        if ((ret < 0) && (ERROR_P2P_TIME_OUT != ret)) {
            //异常处理
            if (ERROR_P2P_SESSION_CLOSED_REMOTE == ret || 
                ERROR_P2P_SESSION_CLOSED_TIMEOUT == ret ||
    ...
    tal_md5_update_ret(md5,(BYTE_T *)(sg_p2p_session->str_P2p_auth.p2p_passwd),strlen(sg_p2p_session->str_P2p_auth.p2p_passwd));
    tal_md5_update_ret(md5,(BYTE_T *)"||",2);
    tal_md5_update_ret(md5,(BYTE_T *)(sg_p2p_session->str_P2p_auth.gw_local_key),strlen(sg_p2p_session->str_P2p_auth.gw_local_key));
    ...
    lk_dm5[offset] = 0;    
    //PR_DEBUG("Client Auth %s %s <-> %s %s ", strUserInfo.user, strUserInfo.passwd, sg_p2p_ctl.str_P2p_auth.p2p_name, sg_p2p_ctl.str_P2p_auth.p2p_passwd);
...
STATIC VOID __p2p_ext_protocol_pack(INT_T client, INT_T type, CHAR_T *p_result, INT_T *p_result_len)
{
    ...
    if (0 == type) {
        ...
        if (TRUE == sg_p2p_session->key_frame) {
            ...
            *(SHORT_T*)&p_result[sizeof(C2C_AV_TRANS_FIXED_HEADER) + 2] = (SHORT_T)sg_p2p_session->av_Info.width[curClirtyChn];
            *(SHORT_T*)&p_result[sizeof(C2C_AV_TRANS_FIXED_HEADER) + 4] = (SHORT_T)sg_p2p_session->av_Info.height[curClirtyChn];
            *(SHORT_T*)&p_result[sizeof(C2C_AV_TRANS_FIXED_HEADER) + 6] = (SHORT_T)sg_p2p_session->av_Info.fps[curClirtyChn];
        } else {
...
STATIC OPERATE_RET __p2p_check_free_buffer_size(INT_T client, INT_T channel, INT_T len)
{
	...
	ret = tuya_p2p_rtc_check_buffer(sg_p2p_session->session, channel, (uint32_t *)&writeSize, NULL, (uint32_t *)&sendFreeSize);
	if (OPRT_OK != ret) {
        return ret;
    }	
    ...
    if (need_size > sendFreeSize) {	
        ...
        if (retry_sum%100 == 0){
            PR_ERR("Check_Buffer not enough writeSize[%d] sendFreeSize[%d] len[%d] session[%d] channel[%d]",\
                writeSize, sendFreeSize, len, sg_p2p_session->session, channel);
        }
        retry_sum++;
...
//             if (OPRT_OK == tuya_ipc_pack_aac_rtp((BYTE_T * )(pData + i + ADTS_HEADER_LENGTH), strAdts.aac_frame_length - ADTS_HEADER_LENGTH,\
...
STATIC INT_T __p2p_session_cmd_parse_server(P2P_SESSION_T* pSession, VOID* pData)
{
    ...
    switch(pFixedHead->high_cmd){
        ...
        case TY_C2C_CMD_QUERY_VIDEO_STREAM_PARAMS:
        {
            ...
            if (NULL != pVideoResp) {
                __p2p_session_pack_resp(pSession,pData,pVideoResp, \
                    sizeof(C2C_TRANS_QUERY_VIDEO_PARAM_RESP_T) + pVideoResp->count*sizeof(VIDEO_PARAM_T));	
                Free(pVideoResp);
            } else {
                //发送失败消息给app
                C2C_CMD_IO_CTRL_COM_RESP_T comResp;
        ...
        case TY_C2C_CMD_IO_CTRL_VIDEO:
        {
            ...
            switch(parm->operation){
                case TY_CMD_IO_CTRL_VIDEO_PLAY:
                {
                 ...
                 //   PR_DEBUG("CTRL VIDEO START session[%d] chn[%d] op[%d]",pSession->session,parm->channel,parm->operation);
                    //20190416add
                    if (0 != parm->channel) {
        ...
        case TY_C2C_CMD_IO_CTRL_VIDEO_CLARITY:
        {
            ...
            outParm.clarity = (parm->mode == TY_VIDEO_CLARITY_INNER_HIGH)?TY_VIDEO_CLARITY_HIGH:TY_VIDEO_CLARITY_STANDARD;
            //outParm.clarity = __p2p_clarity_trans(parm->mode);
            PR_DEBUG("set video clarity session[%d]chn[%d] op[%d] clarity[%d]",pSession->session,parm->channel,parm->mode,outParm.clarity);
...
STATIC INT_T __p2p_read_cmd(P2P_SESSION_T *pSession)
{
    ...
    ret = tuya_p2p_rtc_recv_data(pSession->session , TUYA_CMD_CHANNEL, pDataParse->read_buff + pDataParse->cur_read, &pDataParse->read_size, P2P_RECV_TIMEOUT);  
    if ((ret < 0) && (ERROR_P2P_TIME_OUT != ret)) {
        //异常处理
        if (ERROR_P2P_SESSION_CLOSED_REMOTE == ret || 
            ERROR_P2P_SESSION_CLOSED_TIMEOUT == ret ||
            ERROR_P2P_SESSION_CLOSED_CALLED == ret ||
            ERROR_P2P_NOT_INITIALIZED == ret ||
            ERROR_P2P_INVALID_SESSION_HANDLE == ret ||
            ERROR_P2P_INVALID_PARAMETER == ret) {
            //session被客户端断开，需要关闭session
        ...
        //PR_DEBUG("recv cmd size[%d] cur_read[%d] flag[%d]",pDataParse->read_size,pDataParse->cur_read,pDataParse->flag); 
        //接收数据解析，确认接收数据完整
    ...
    while (tal_thread_get_state(sg_p2p_session->video_send_proc_thread) == THREAD_STATE_RUNNING)
	{
        ...
        if (P2P_AUDIO & cmd) {
            ...
            if (op_ret == OPRT_OK) {
                ...
                } else if (TY_AV_CODEC_AUDIO_G711A == type || TY_AV_CODEC_AUDIO_G711U == type || TY_AV_CODEC_AUDIO_PCM == type) {
...
OPERATE_RET p2p_init(IN CONST TUYA_IPC_P2P_VAR_T *p_var)
{
    ...
    ret = tal_thread_create_and_start(&(sg_p2p_session->cmd_recv_proc_thread),NULL,NULL,__p2p_cmd_recv_proc,NULL,&thrd_param);
    if(ret != OPRT_OK) {
    ...
    ret = tal_thread_create_and_start(&(sg_p2p_session->video_send_proc_thread),NULL,NULL,__p2p_media_send_proc,NULL,&thrd_param);
    if(ret != OPRT_OK) {
