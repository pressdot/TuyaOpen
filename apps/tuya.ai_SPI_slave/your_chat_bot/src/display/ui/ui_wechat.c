...
typedef struct {
    ...
    lv_obj_t *stm32_ip_bubble;
    lv_obj_t *server_ip_bubble;
    lv_obj_t *stm32_ip_label;
    lv_obj_t *server_ip_label;
} APP_UI_T;
...
int ui_init(UI_FONT_T *ui_font)
{
    ...
    lv_obj_set_style_bg_color(sg_ui.ui.status_bar, lv_palette_main(LV_PALETTE_BLUE), 0);
    ...
    // Create a container for STM32 IP and remote server IP bubbles
    lv_obj_t *ip_container = lv_obj_create(sg_ui.ui.container);
    lv_obj_set_size(ip_container, LV_HOR_RES, 240);
    lv_obj_set_style_bg_color(ip_container, lv_color_hex(0xFFFFFF), 0);
    lv_obj_set_style_border_width(ip_container, 0, 0);
    lv_obj_set_style_pad_all(ip_container, 8, 0);

    // 使用flex布局让两个圆形水平且垂直居中排列
    lv_obj_set_flex_flow(ip_container, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(ip_container, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
    lv_obj_set_style_pad_column(ip_container, 40, 0);

    // STM32 IP "bubble"
    lv_obj_t *stm32_ip_bubble = lv_obj_create(ip_container);
    lv_obj_remove_style_all(stm32_ip_bubble);
    lv_obj_set_size(stm32_ip_bubble, 200, 200); // 放大
    lv_obj_set_style_radius(stm32_ip_bubble, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(stm32_ip_bubble, lv_palette_lighten(LV_PALETTE_BLUE, 3), 0);
    lv_obj_set_style_border_width(stm32_ip_bubble, 2, 0);
    lv_obj_set_style_border_color(stm32_ip_bubble, lv_palette_main(LV_PALETTE_BLUE), 0);

    // Server IP "bubble"
    lv_obj_t *server_ip_bubble = lv_obj_create(ip_container);
    lv_obj_remove_style_all(server_ip_bubble);
    lv_obj_set_size(server_ip_bubble, 200, 200); // 放大
    lv_obj_set_style_radius(server_ip_bubble, LV_RADIUS_CIRCLE, 0);
    lv_obj_set_style_bg_color(server_ip_bubble, lv_palette_lighten(LV_PALETTE_GREEN, 3), 0);
    lv_obj_set_style_border_width(server_ip_bubble, 2, 0);
    lv_obj_set_style_border_color(server_ip_bubble, lv_palette_main(LV_PALETTE_GREEN), 0);

    lv_obj_t *stm32_ip_label = lv_label_create(stm32_ip_bubble);
    lv_label_set_text(stm32_ip_label, "STM32\nIP:PORT");
    lv_obj_set_style_text_align(stm32_ip_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_center(stm32_ip_label);

    lv_obj_t *server_ip_label = lv_label_create(server_ip_bubble);
    lv_label_set_text(server_ip_label, "Server\nIP:PORT");
    lv_obj_set_style_text_align(server_ip_label, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_center(server_ip_label);

    // 让ip_container在父容器中水平和垂直居中
    lv_obj_center(ip_container);

    // Save pointers if you want to update them later
    sg_ui.ui.stm32_ip_bubble = stm32_ip_bubble;
    sg_ui.ui.server_ip_bubble = server_ip_bubble;

    // 保存label指针以便后续修改
    sg_ui.ui.stm32_ip_label = stm32_ip_label;
    sg_ui.ui.server_ip_label = server_ip_label;

    return 0;
}

// 添加：修改STM32 IP label文字
void ui_set_stm32_ip_text(const char *text)
{
    if (sg_ui.ui.stm32_ip_label) {
        lv_label_set_text(sg_ui.ui.stm32_ip_label, text);
    }
}

// 添加：修改Server IP label文字
void ui_set_server_ip_text(const char *text)
{
    if (sg_ui.ui.server_ip_label) {
        lv_label_set_text(sg_ui.ui.server_ip_label, text);
    }
}
