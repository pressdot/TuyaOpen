...
OPERATE_RET httpc_dev_update_skill_v10(IN CONST CHAR_T *gw_id, IN CONST CHAR_T *sub_id, IN CONST CHAR_T *skill)
{
    ...
    if(sub_id == NULL)
    {
        ...
        snprintf(post_data, buffer_len, "{\"skill\":\"%s\",\"t\":\"%d\"}", skill, timestamp);//注意：tuya-open要在这里加个时间戳
    }else
    {
...
OPERATE_RET tuya_ipc_cloud_gw_kills_get(UINT_T device,UINT_T channel ,CHAR_T * skill_result)
{
    ...
    snprintf(skill_result+len, 512,
        ...
        media_info.av_encode_info.video_codec[E_IPC_STREAM_VIDEO_MAIN], media_info.av_encode_info.video_freq[E_IPC_STREAM_VIDEO_MAIN],
        media_info.av_encode_info.video_width[E_IPC_STREAM_VIDEO_MAIN],media_info.av_encode_info.video_height[E_IPC_STREAM_VIDEO_MAIN],
        media_info.av_encode_info.video_codec[E_IPC_STREAM_VIDEO_SUB],media_info.av_encode_info.video_freq[E_IPC_STREAM_VIDEO_SUB],
        media_info.av_encode_info.video_width[E_IPC_STREAM_VIDEO_SUB],media_info.av_encode_info.video_height[E_IPC_STREAM_VIDEO_SUB],
        media_info.av_encode_info.audio_codec[E_IPC_STREAM_AUDIO_MAIN],media_info.av_encode_info.audio_sample[E_IPC_STREAM_AUDIO_MAIN],
