...
bool pj_ice_session_create(pj_ice_session_cfg_t *pCfg, pj_ice_session_t **ppIceSession)
{
	...
	//pj_thread_create(pIceSession->pPool, "ice_worker_thread", &ice_worker_thread, pIceThreadParam, 0, 0, &pIceSession->pThread);
    // pj_str_t szDNSServers[2];
	// szDNSServers[0] = pj_str((char*)"8.8.8.8");
	// szDNSServers[1] = pj_str((char*)"144.144.144.144");
	// pj_dns_resolver_create(&pIceCfg->cachePool.factory, "resolver", 0, pIceCfg->iceCfg.stun_cfg.timer_heap, pIceCfg->iceCfg.stun_cfg.ioqueue, &pIceCfg->iceCfg.resolver);
	// pj_dns_resolver_set_ns(pIceCfg->iceCfg.resolver, 1, szDNSServers, NULL);
...
bool pj_ice_session_init(pj_ice_session_t *pIceSession, pj_ice_session_cfg_t *pCfg)
{
	...
	/* create the instance */
	pj_status_t status = pj_ice_strans_create("icedemo", &pIceSession->iceCfg, pIceSession->uComponentCount, pCfg->user_data, &icecb, &pIceSession->pIceSTransport);
    if (status != PJ_SUCCESS)
    {
        return false;
    }
...
bool pj_ice_session_add_remote_candidate(pj_ice_session_t *pIceSession, pj_str_t *rem_ufrag, pj_str_t *rem_passwd, unsigned rcand_cnt, pj_ice_sess_cand rcand[], pj_bool_t rcand_end)
{
...
bool pj_ice_session_sendto(pj_ice_session_t *pIceSession, void *pkt, uint32_t len)
{
    ...
    status = pj_ice_strans_sendto2(ice_st, comp_id, pkt, len, &pIceSessCheck->rcand->addr, pj_sockaddr_get_len(&pIceSessCheck->rcand->addr));
