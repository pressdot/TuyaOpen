...
int32_t tuya_p2p_rtp_session_init( tuya_p2p_rtp_session_t *ses,
					       int default_pt, 
					       uint32_t sender_ssrc );


int32_t tuya_p2p_rtp_session_init2( 
				    tuya_p2p_rtp_session_t *ses,
				    tuya_p2p_rtp_session_setting settings);

int32_t tuya_p2p_rtp_encode_rtp( tuya_p2p_rtp_session_t *ses, 
					     int pt, int m,
					     int payload_len, int ts_len,
					     const void **rtphdr, 
					     int *hdrlen );

int32_t tuya_p2p_rtp_decode_rtp( tuya_p2p_rtp_session_t *ses, 
					     const void *pkt, int pkt_len,
					     const tuya_p2p_rtp_hdr_t **hdr,
					     const void **payload,
					     unsigned *payloadlen);

int32_t tuya_p2p_rtp_decode_rtp2(
				    tuya_p2p_rtp_session_t *ses,
				    const void *pkt, int pkt_len,
				    const tuya_p2p_rtp_hdr_t **hdr,
				    tuya_p2p_rtp_dec_hdr_t *dec_hdr,
				    const void **payload,
				    unsigned *payloadlen);

void tuya_p2p_rtp_session_update( tuya_p2p_rtp_session_t *ses, 
					  const tuya_p2p_rtp_hdr_t *hdr,
					  tuya_p2p_rtp_status_t *seq_st);


void tuya_p2p_rtp_session_update2(tuya_p2p_rtp_session_t *ses, 
					  const tuya_p2p_rtp_hdr_t *hdr,
					  tuya_p2p_rtp_status_t *seq_st,
					  bool check_pt);

void tuya_p2p_rtp_seq_init(tuya_p2p_rtp_seq_session_t *seq_ctrl, 
			  uint16_t seq);

void tuya_p2p_rtp_seq_update( tuya_p2p_rtp_seq_session_t *seq_ctrl, 
			     uint16_t seq,
			     tuya_p2p_rtp_status_t *seq_status);
